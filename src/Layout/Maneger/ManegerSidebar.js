...
export default function ManegerSidebar() {
  ...
  return (
    ...
    <div className='flex flex-col my-5 items-end justify-between'>
        ...
        <div className='text-white flex flex-col space-y-10 items-end'>
          ...
          <div>
            <Link className={`${(location.pathname==='/ManagerDashboard')?'text-orange-500':''}`} to="">صندوق</Link>
          </div>
          <div>
            <Link className={`${(location.pathname==='/ManagerDashboard/Users')?'text-orange-500':''}`} to="Users">کاربران</Link>
          </div>
          <div>
            <Link className={`${(location.pathname==='/ManagerDashboard/Loan')?'text-orange-500':''}`} to="Loan">وام ها</Link>
          </div>
          <div>
            <Link className={`${(location.pathname==='/ManagerDashboard/Reports')?'text-orange-500':''}`} to="Reports">گزارشات</Link>
          </div>
          <div>
            <Link className={`${(location.pathname==='/ManagerDashboard/Connections')?'text-orange-500':''}`} to="Connections">ارتناطات</Link>
          </div>
